...
func (m msgServer) CreatePool(goCtx context.Context, msg *types.MsgCreatePool) (*types.MsgCreatePoolResponse, error) {
	...
	if maxRewardCategories := m.Keeper.MaxRewardCategories(ctx); uint32(len(msg.TotalRewards)) > maxRewardCategories {
		return nil, sdkerrors.Wrapf(
			types.ErrInvalidRewardRule,
			"the max reward category num is [%d], but got [%d]",
			maxRewardCategories, len(msg.TotalRewards),
		)
	}
	//check pool exist
	...
	if err := m.Keeper.validateLPToken(ctx, msg.MinStaking.Denom); err != nil {
		return nil, sdkerrors.Wrapf(
			types.ErrInvalidLPToken,
			"The lp token denom[%s] is not exist",
			msg.MinStaking.Denom,
		)
	}
	if err = m.Keeper.CreatePool(
		...
		msg.MinStaking,
		msg.StartHeight,
		msg.RewardsPerBlock.Sort(),
		msg.TotalRewards.Sort(),
		creator,
	); err != nil {
		return nil, err
	}
...
func (m msgServer) Unbond(goCtx context.Context, msg *types.MsgUnbond) (*types.MsgUnbondResponse, error) {
	...
	ctx := sdk.UnwrapSDKContext(goCtx)
	reward, err := m.Keeper.Unbond(ctx, msg.PoolName, msg.Amount, sender)
	if err != nil {
		return nil, err
	}
	ctx.EventManager().EmitEvents(sdk.Events{
...
func (m msgServer) Withdraw(goCtx context.Context, msg *types.MsgWithdraw) (*types.MsgWithdrawResponse, error) {
	...
	ctx := sdk.UnwrapSDKContext(goCtx)
	reward, err := m.Keeper.Withdraw(ctx, msg.PoolName, sender)
	if err != nil {
		return nil, err
	}
	ctx.EventManager().EmitEvents(sdk.Events{
