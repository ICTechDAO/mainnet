...
func (k BaseSendKeeper) hasToken(ctx sdk.Context, token types.Token) bool {
	...
	// Low efficiency?
	issuedTokens := k.bankKeeper.GetSupply(ctx, token.GetSymbol()).Amount
	if issuedTokens.GT(sdk.ZeroInt()) {
		return true
	}
	issuedTokens = k.bankKeeper.GetSupply(ctx, token.GetSmallestUnit()).Amount
	if issuedTokens.GT(sdk.ZeroInt()) {
		return true
	}
