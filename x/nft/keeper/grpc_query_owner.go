...
func (k Keeper) Owner(goCtx context.Context, req *types.QueryOwnerRequest) (*types.QueryOwnerResponse, error) {
	...
	if req.CateId == "" {
		key = types.KeyPrefix(types.PrefixOwner + req.Owner)
	} else {
		key = types.KeyPrefix(types.PrefixOwner + req.Owner + req.CateId)
	}
	...
	return &types.QueryOwnerResponse{Nfts: nfts, Pagination: pageRes}, nil
}
